...
class BanStickMod(loader.Module):
    ...
    strings = {"name": "StickBan"}

    async def client_ready(self, client, db):
        self.db = db

    async def stickbancmd(self, message):
        """Используй: .stickban чтобы заблокировать стикер или гифку в чате; аргументы clear или clearall (по желанию)"""
        banned = self.db.get("StickBan", "banned", {})
        ...
        try:
            if reply.media:
                ...
                if str(docid) not in banned[chat]:
                    ...
                    self.db.set("StickBan", "banned", banned)
                    await message.edit(
